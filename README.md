# CloudNav-ioing v1.8 更新说明
新增：云端同步手动开关

1. 在「设置 → 网站设置」中新增云端自动同步开关，默认开启（不影响原有体验）
2. 关闭同步后，所有数据修改仅保存在本地浏览器，不会自动推送至 Cloudflare KV 云端
3. 关闭状态下底部状态栏显示「同步关闭」（灰色），替代之前不准确的「已同步」
4. 开启同步时自动触发一次全量推送，确保关闭期间积累的本地数据不会因刷新而丢失
5. 同步关闭状态下提供**「手动同步到云端」**按钮，可一键强制推送当前数据
6. 开关切换即时生效并保存至 KV，无需点击「保存更改」
